// DemoLib.cpp : 此文件包含 "main" 函数。程序执行将在此处开始并结束。
//
#include <iostream>
#include <io.h>
#include <stdio.h>
...
int main()
{
	...
	* Get project path
	*/
	memset(tPath, 0x00, MAX_PATH);
	::GetModuleFileName(NULL, tPath, MAX_PATH);
	(wcsrchr(tPath, TEXT('\\')))[1] = 0;

	int len = 0;
	memset(szPath, 0x00, MAX_PATH);
	len = WideCharToMultiByte(CP_ACP, 0, tPath, -1, NULL, 0, NULL, NULL);
	WideCharToMultiByte(CP_ACP, 0, tPath, -1, szPath, len, NULL, NULL);
	...
	*/
	printf("1、InitDll\n");
	pFpd = HBI_Init(0); // default detector id is 0
	if (pFpd == NULL)
	{
	...
	*/
	...
	ret = HBI_RegEventCallBackFun(pFpd, handleCommandEventA, NULL/*(void *)this*/); // the pointer of position machine object,Can be null ptr
	if (0 != ret)
	{
...
//	commCfg._type = FPD_COMM_TYPE::WALN_COMM_TYPE;
	...
	j = sprintf(commCfg._localip, "%s", "192.168.10.20");
	commCfg._localip[j] = '\0';
	j = sprintf(commCfg._remoteip, "%s", "192.168.10.40");
	commCfg._remoteip[j] = '\0';
	ret = HBI_ConnectDetector(pFpd, commCfg, dooffsetTemplate);
	if (0 != ret)
	{
	...
	*/
	...
	ret = HBI_TriggerAndCorrectApplay(pFpd, triggermode, pcorrect);
	if (0 != ret)
	{
	...
	while ((ch1 = getchar()) != 'q')
	{
		...
		case '1':// single shot mode2 
			...
			ret = HBI_SingleAcquisition(pFpd, aqc_mode);
			if (ret != 0)
				printf("HBI_SingleAcquisition failed!\n");
			else
				printf("HBI_SingleAcquisition success!\n");
			break;
		case '2':// live acquisition	
			...
			aqc_mode.ngroupno = 0;
			aqc_mode.ndiscard = 0;
			ret = HBI_LiveAcquisition(pFpd, aqc_mode);
			if (ret != 0)
				printf("HBI_LiveAcquisition failed!\n");
			else
				printf("HBI_LiveAcquisition success!\n");
			break;
		case '3':// stop live acquisition	
			ret = HBI_StopAcquisition(pFpd);
			if (ret != 0)
				printf("HBI_StopAcquisition failed!\n");
			else
				printf("HBI_StopAcquisition success!\n");
			printf("\nPlease enter:");
			break;
		case '4': // get the parameters of the detector firmware
			ret = HBI_GetFpdCfgInfo(pFpd, pLastRegCfg);
			if (0 != ret)
			{
		...
		case '5': // Get image properties
			ret = HBI_GetImageProperty(pFpd, &img_pro);
			if (ret == 0)
			{
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////
		case '6': // Generate offset template.
			...
			ret = HBI_GenerateTemplate(pFpd, enumTemplateType);
			if (ret == HBI_SUCCSS)
				printf("Offset-HBI_GenerateTemplate succss!\n");
			else
				printf("err:Offset-HBI_GenerateTemplate failed!\n");
			break;
		...
		case '7': // Generate gain template.Under normal dose, collect a group of bright field images.
			...
			if (ret == HBI_SUCCSS)
			{
				printf("Gain-HBI_GenerateTemplate succss!\n");

				// Upload template file to firmware
				if (0 != HBI_RegProgressCallBack(pFpd, DownloadCallBackFun, NULL/*(void *)this*/)) //    
				{
					printf("err:Gain-HBI_RegProgressCallBack failed!\n");
					break;
				}
				else
					printf("Gain-HBI_RegProgressCallBack success!\n");

				// int infiletype - Template file type
                // 0-gain template，1-defect template，2-offset template，others-invalid.
				infiletype = 0;
				int ret = HBI_DownloadTemplateByType(pFpd, /*0*/infiletype);
				if (ret != HBI_SUCCSS)
				...
				printf("err:Gain-HBI_GenerateTemplate failed!\n");
		...
		case 'a': // Generate defect template-Collect the third group of bright field images and generate templates.
			...
			if (ret == HBI_SUCCSS)
			{
				printf("Defect group3-HBI_GenerateTemplate succss!\n");
				...
				if (0 != HBI_RegProgressCallBack(pFpd, DownloadCallBackFun, NULL/*(void *)this*/)) //    
				{
					...
					printf("defect-HBI_RegProgressCallBack success!\n");

				// int infiletype - Template file type
				// 0-gain template，1-defect template，2-offset template，others-invalid
				infiletype = 1;
				int ret = HBI_DownloadTemplateByType(pFpd, /*1*/infiletype);
				if (ret != HBI_SUCCSS)
				...
				printf("err:Defect group3-HBI_GenerateTemplate failed!\n");
			//printf("Close the X-ray tube after completion.\n");
			break;
		case 'b': // Update calibration enable status.
			if (pcorrect != NULL)
			{
				...
				pcorrect->ucDefectCorrection = 0x02;
				pcorrect->ucDummyCorrection  = 0x00;
				ret = HBI_UpdateCorrectEnable(pFpd, pcorrect);
				if (ret != HBI_SUCCSS)
					printf("err:HBI_UpdateCorrectEnable failed!\n");
				else
					printf("HBI_UpdateCorrectEnable success!\n");
			}
			break;
//////////////////////////////////////////////////////////////////////////////////////////////////////////////
// Generate calibration template
...
EXIT_EXIT_MAIN:
	...
	if (pFpd != NULL)
	{
		// HBI_DisConnectDetector(pFpd);/* disconnect detector */
		HBI_Destroy(pFpd);    // include DisConnectDetector and free dll resource
		pFpd = NULL;
	}
	if (pLastRegCfg != NULL) {
		delete pLastRegCfg;
...
// Main callback function:Static member function or global function.
