...
// Convert from vector to double pointer format
double** convertToDoublePtr(const std::vector<std::vector<uint16_t>>& input) {
    ...
    for(int i = 0; i < height; i++) {
        ...
        for(int j = 0; j < width; j++) {
            output[i][j] = static_cast<double>(input[i][j]);
        }
    }
    return output;
}
...
std::vector<std::vector<uint16_t>> convertToVector(double** input, int height, int width) {
    std::vector<std::vector<uint16_t>> output(height, std::vector<uint16_t>(width));
    for(int i = 0; i < height; i++) {
        for(int j = 0; j < width; j++) {
            output[i][j] = static_cast<uint16_t>(std::round(input[i][j]));
        }
    }
    return output;
}

std::vector<std::vector<uint16_t>> ControlPanel::convertToVector(double** input, int height, int width) {
    std::vector<std::vector<uint16_t>> output(height, std::vector<uint16_t>(width));
    for(int i = 0; i < height; i++) {
        for(int j = 0; j < width; j++) {
            // Clamp values to uint16_t range
            double value = input[i][j];
            value = std::clamp(value, 0.0, 65535.0);
            output[i][j] = static_cast<uint16_t>(std::round(value));
        }
    }
    return output;
}
